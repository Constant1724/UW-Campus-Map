...
/**
 * A simple text user interface allowing client to list buildings find path between buildings print
 * menu quit the program
 *
 * <p>through command line text interface.
 */
public class UserInterface {
  ...
  /**
   * Standard main method. read user input from System.in and perform corresponding functions and
   * output result to System.out
   *
   * <p>b lists all buildings in the form abbreviated name: long name. Buildings are listed in
   * alphabetical order of abbreviated name. r prompts the user for the abbreviated names of two
   * buildings and prints directions for the shortest route between them. q quits the program. m
  ...
  public static void main(String[] args) {
    ...
    for (Building building : model.listBuildings()) {
      ...
      formattedBuilding.add(
          String.format("\t%s: %s", building.getShortName(), building.getLongName()));
    }

    // Sort the formattedBuilding based on the alphabetical order of shortName.
    formattedBuilding.sort(
        (o1, o2) -> {
    ...
    // Quick sanity check.
    assert formattedBuilding.size() == shortToBuilding.size()
        && shortToBuilding.size() == model.listBuildings().size();
   ...
   *
   * <p>Menu: r to find a route b to see a list of all buildings q to quit
   */
  ...
  public static void printPaths(
      Scanner reader, Map<String, Building> shortToBuilding, CampusMapModel model) {
    ...
    System.out.println(
        String.format(
            "Path from %s to %s:", startBuilding.getLongName(), endBuilding.getLongName()));

    List<CampusPath> results =
        model.findPath(startBuilding.getLocation(), endBuilding.getLocation());
  ...
  /**
   * Format a CampusPath into: Walk CampusPath.getCost() feet someDirection to
   * (CampusPath.getDestination.getX(), CampusPath.getDestination.getY())
   *
   * <p>For example: Walk 36 feet NW to (2187, 950)
   *
   * <p>All number should round to the nearest integer.
   *
  ...
  private static String formatPath(CampusPath path) {
    ...
    String verboseStep =
        String.format("\tWalk %.0f feet %s to (%.0f, %.0f)", path.getCost(), direction, endX, endY);
    return verboseStep;
  }
}

...
public class MarvelPaths2 {
  ...
  public static void main(String[] args) {
    ...
    while (true) {
      ...
      Graph<String, Double>.Node startNode =
          graph.makeNode(MarvelPaths.readInput(reader, "character1: "));
      Graph<String, Double>.Node endNode =
          graph.makeNode(MarvelPaths.readInput(reader, "character2: "));
   ...
   *
   * @spec.requires graph != null and start != null and end != null and all edges should have
   *     non-negative label.
  ...
  public static <N extends @NonNull Object> @Nullable List<Graph<N, Double>.Edge> findPath(
    ...
    // Note that since the number of edges from start to some node is << than the total number of
    // edges in graph,
    // We may just loop through the edges and sum them up.
    // It holds path from start to some node as lists of edges. Note that the path is not
    // necessarily the minPath.
    Queue<List<Graph<N, Double>.Edge>> active =
        new PriorityQueue<>(Comparator.comparing(MarvelPaths2::sumCost));
    ...
    while (!active.isEmpty()) {
      ...
      // As a result, we prove by induction that any edge in a list in active must be an edge in
      // graph.
      // The Graph ADT requires that if an edge is in the graph, both start and end must be in the
      // graph as well.
